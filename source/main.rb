...
categorias = page.search(".seccion_texto ul ul li").map do |li|
  ...
  info_link = li.search('a').map do |a|
    ...
    informes = page_informes.search(".seccion_texto ul li").map do |li|
	  ...
	  info_link = li.search('a').map do |a|
        begin 
		  ...
		  docNum = getDocNumber(textLink, docType, docDate)
		   ...
		   #log########################
		  logStatistics.debug "-------------------------------------------------------------"
		  logStatistics.debug textLink
		  logStatistics.debug docDate
		  
		  if docDate == "yyyy-mm-dd"
			docsUndated += 1
		  else
		  ...
		  if (deliveryType != "update" || isNewDocument(logBook,categoria,docDate))
			...
			createMetadataXML(docNum, docDate, title, docType.capitalize, valueIdDT, subjectDescription, subjectId, docId, url, docNumFolderName, fileName)
